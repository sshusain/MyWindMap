...
var Animator = function(element, opt_animFunc, opt_unzoomButton) {
	...
	if (element) {
  	...
  	$(element).mousedown(function(e){
			self.mouseX = e.pageX - this.offsetLeft-59;
	    self.mouseY = e.pageY - this.offsetTop-10;
		display.startRect(self.mouseX, self.mouseY)
  		self.mousedown();
		//console.log("offset: "+this.offsetLeft+" pageX: "+e.pageX);
...
Animator.prototype.mousemove = function() {
	...
	display.endRect(this.mouseX, this.mouseY);
...
MotionDisplay.prototype.startRect=function(startX, startY)
{
	this.rectX1=startX;
	this.rectY1=startY;
	this.rectX2=startX;
...
MotionDisplay.prototype.endRect=function(endX, endY)
{
	//console.log(this.rectY2-this.rectY1)
	this.rectX2=endX;
	this.rectY2=endY;
}
...
MotionDisplay.prototype.makeParticle = function(animator) {
	...
	for (;;) {
		...
		if (this.field.maxLength == 0) {
			return new Particle(x, y, 1 + 100 * Math.random());
		}
...
MotionDisplay.prototype.startMove = function(animator) {
	// Save screen.
	console.log(document.getElementById("city-display").pageX)
};
...
MotionDisplay.prototype.draw = function(animator) {
	...
	g.fillRect(dx, dy, w * scale,h * scale);
	g.fillStyle="rgba(50,100,100,.1)"
	g.fillRect(this.rectX1, this.rectY1, this.rectX2-this.rectX1, this.rectY2-this.rectY1)
	...
	for (var i = 0; i < this.particles.length; i++) {
		...
		if(!p.inBox)
		{
		var xComp=Math.abs(this.rectX2-this.rectX1)
		var yComp=Math.abs(this.rectY2-this.rectY1)
		if(Math.abs(proj.x-this.rectX1)<xComp&&Math.abs(proj.x-this.rectX2)<xComp&&Math.abs(proj.y-this.rectY1)<yComp&&Math.abs(proj.y-this.rectY2)<yComp)
			p.inBox=true;
		}
